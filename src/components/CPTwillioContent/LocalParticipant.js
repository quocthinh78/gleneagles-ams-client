...
export default memo(function LocalParticipant({
  participant,
  ...
  useEffect(() => {
    ...
    participant.on("trackDisabled", (a) => {
      // console.log("trackDisabled ", a, a.trackSid === undefined, a.trackSid);
      if (a.trackSid === undefined) updateTrackStatus(a);
    });
    participant.on("trackEnabled", (a) => {
      // console.log("trackEnabled ", a, a.trackSid === undefined, a.trackSid);
      if (a.trackSid === undefined) updateTrackStatus(a);
    });
  ...
  const addTrack = (addedTrack) => {
    if (addedTrack.name === SHARE_SCREEN_TRACK_NAME) {
      ...
      onConnectedScreenTrack(addedTrack, true);
    } else {
      addLocalTrack(addedTrack);
      dispatch({ type: ADD_TRACK, payload: addedTrack });
      if (addedTrack.kind === "audio")
        dispatch({
          type: SET_AUDIO_STATUS,
          payload: { isEnabled: addedTrack.isEnabled, id: addedTrack.id },
        });
  ...
  const removeTrack = (removedTrack) => {
    if (removedTrack.name === SHARE_SCREEN_TRACK_NAME) {
      ...
      onDisconnectedScreenTrack(removedTrack, true);
    } else {
      removeLocalTrack(removedTrack);
      dispatch({ type: REMOVE_TRACK, payload: removedTrack });
      if (removedTrack.kind === "audio")
        dispatch({
          type: SET_AUDIO_STATUS,
          payload: { isEnabled: removedTrack.isEnabled, id: removedTrack.id },
        });
  ...
  const updateTrackStatus = (a) => {
    if (a.kind === "audio")
      dispatch({
        type: SET_AUDIO_STATUS,
        payload: { isEnabled: a.isEnabled, id: a.id },
      });
